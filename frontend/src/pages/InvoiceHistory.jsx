...
import { FileText, TrendingUp, CheckCircle, Clock, IndianRupee, ChevronDown } from 'lucide-react'
...
export default function InvoiceHistory() {
  ...
  useEffect(() => {
    Promise.all([api.get('/invoice/history'), api.get('/invoice/stats')])
      .then(([h, s]) => { setInvoices(h.data); setStats(s.data) })
    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 animate-fade-up delay-100">
          <StatCard icon={TrendingUp} label="Total Earned" value={fmt(stats.total)} sub={`${stats.count} invoices`} color="gold" />
            ...
            {stats.monthly.map(m => {
              ...
              return (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[9px] text-ink-400 font-mono">{fmt(m.amount, 'USD').replace('US$', '$')}</span>
                  <div className="w-full bg-gold-400/20 rounded-t" style={{ height: `${Math.max(pct, 4)}%` }}>
                    <div className="w-full h-full bg-gold-400 rounded-t opacity-80" />
                  </div>
      ...
      <div className="bg-white/80 border border-ink-200 rounded-xl overflow-hidden animate-fade-up delay-200">
        ...
        {filtered.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((inv, i) => (
                <tr key={inv.id} className={`border-b border-ink-50 hover:bg-ink-50/50 transition-colors ${i === filtered.length - 1 ? 'border-0' : ''}`}>
                  <td className="px-5 py-3.5 font-mono text-xs text-ink-600">{inv.invoice_number}</td>
                  <td className="px-5 py-3.5 text-ink-800 font-medium">{inv.client_name || '—'}</td>
                  <td className="px-5 py-3.5 font-mono font-semibold text-ink-800">{fmt(inv.amount, inv.currency)}</td>
                  <td className="px-5 py-3.5 text-ink-500">{inv.issue_date || '—'}</td>
