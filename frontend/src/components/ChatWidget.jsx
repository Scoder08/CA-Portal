...
// Quick-start suggestion chips shown on the welcome screen
const SUGGESTIONS = [
...
// Handles: ```fenced code```, **bold**, *italic*, `inline code`,
//          ---, # headings, - lists, newlines
// ---------------------------------------------------------------------------
...
function MarkdownContent({ content, isUser, streaming }) {
  ...
  // Split content into fenced code blocks and normal text blocks first
  const segments = []
  const fenceRe = /```(\w*)\n?([\s\S]*?)```/g
  let lastIdx = 0, fm
    ...
    <div className="py-4 px-1">
      <div className="text-center mb-4">
        ...
        <p className="text-xs text-ink-400 mt-0.5">
          Tax, invoices, GST, FEMA — ask anything
        </p>
      </div>
...
export default function ChatWidget() {
  ...
  // Don't render at all when logged out
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      while (true) {
        ...
        buffer = events.pop() // keep potentially incomplete last chunk
        ...
        for (const event of events) {
          ...
          // Try parsing as JSON (meta / error events)
          try {
      ...
      if (err.name === 'AbortError') return // user triggered new chat
      toast.error(err.message || 'Connection error. Please try again.')
      // Remove the optimistic user message if we never got a response
      if (firstToken) {
        setMessages(prev => prev.filter(m => m.id !== userMsgId))
      }
    } finally {
      // Mark streaming done, show unread badge if panel is closed
      setMessages(prev => {
        const last = prev[prev.length - 1]
        if (last?.role === 'assistant' && last.streaming) {
      ...
      >
        ...
        {!isOpen && hasUnread && (
          <span className="
            ...
            w-3.5 h-3.5 rounded-full bg-red-500
            ...
            <div className="flex items-center gap-3">
              {messages.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 text-ink-300 hover:text-gold-400 transition-colors text-xs"
                >
                  <Plus size={12} /> New
                </button>
              )}
          ...
          <div className="px-3 pb-3 pt-2 border-t border-ink-100 flex-shrink-0">
            <div className="flex gap-2 items-end">
              ...
              <button
                ...
                className="
                  btn-primary p-2.5 rounded-xl flex-shrink-0
